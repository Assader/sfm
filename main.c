...
void kEnter(sfmPart *part, char *tEditor, char *term, int mRow, sfmFiletype **ftypes, int numbOfFTypes);
...
void readConf(sfmPart *fPart, sfmPart *sPart, char *tEditor, char *term, int *keys, char *loc, sfmBind ***ubinds, int *numbOfBinds,
...
void reSize(sfmWindowStruct *fWind, sfmWindowStruct *sWind, int *mRow, int *mCol);
...
int main(int argc, char **argv){
    ...
    while (inCy){
        ...
        if (cmd == KEY_UP)
            ...
            free(ftypes);
            lPart.sWind.currentLine = 0; rPart.sWind.currentLine = 0;
            lPart.sWind.top = 0; rPart.sWind.top = 0;
            lPart.sFold.numbOfFiles = 0; rPart.sFold.numbOfFiles = 0;
...
void draw(sfmPart *part, int mRow, int mCol){
    ...
    while ((tY < mRow - 4)&&(tY < part->sFold.numbOfFiles)){
        mvwprintw(part->sWind.win, tY + 1, 1, "%.*s", mCol / 2 - 3, part->sFold.files[part->sWind.top + tY]->d_name);
        sprintf(fTmp, "%s%s", part->sFold.path, part->sFold.files[part->sWind.top + tY++]->d_name);
        stat(fTmp, &fStat);
...
void fillList(sfmPart *part){
    ...
    part->sFold.numbOfFiles = scandir(part->sFold.path, &part->sFold.files, (part->sFold.showHidden)? 0 : slt, alphasort);
}
...
void getFileTypes(sfmFiletype ***ftypes, int *numbOfFTypes, dictionary *ini){
    ...
    *ftypes = (sfmFiletype **) malloc(sizeof(sfmFiletype *));
    ...
    while (tmp){
        ...
        if ((*numbOfFTypes) > 0)
            *ftypes = (sfmFiletype **) realloc(*ftypes, sizeof(sfmFiletype *) * ((*numbOfFTypes) + 1));
...
void getInfo(sfmPart *part, sfmWindowStruct *wind, int mCol, int mRow){
    ...
    wclear(wind->win);
    box(wind->win, 0, 0);
    sprintf(fTmp, "%s%s", part->sFold.path, part->sFold.files[part->sWind.currentLine]->d_name);
    stat(fTmp, &fStat);
    mvwprintw(wind->win, tY, 1, "Name: %s", part->sFold.files[part->sWind.currentLine]->d_name);
    tY += (strlen(part->sFold.files[part->sWind.currentLine]->d_name)+6)/(mCol/2) + 1;
    ...
    wprintw(wind->win, "(%o)", (fStat.st_mode & S_IRWXU)+(fStat.st_mode & S_IRWXG)+(fStat.st_mode & S_IRWXO));
...
void getUserBinds(sfmBind ***ubinds, int *numbOfBinds, dictionary *ini){
    ...
    *ubinds = (sfmBind **) malloc(sizeof(sfmBind *));
    ...
    for (i = 0;i < 26;i++){
        ...
        if (fTmp[0] != '\0'){
            if ((*numbOfBinds) > 0)
                *ubinds = (sfmBind **) realloc(*ubinds, sizeof(sfmBind *) * ((*numbOfBinds) + 1));
...
void goToLine(sfmPart *part, int mRow){
    ...
    noecho();
    if ((--part->sWind.currentLine < 0) || (part->sWind.currentLine > part->sFold.numbOfFiles)){
        part->sWind.top = 0;
...
void kEnter(sfmPart *part, char *tEditor, char *term, int mRow, sfmFiletype **ftypes, int numbOfFTypes){
    ...
    if (part->sFold.numbOfFiles){
        ...
        stat(fTmp, &fStat);
        if (S_ISDIR(fStat.st_mode)){
            strcat(part->sFold.path, part->sFold.files[part->sWind.currentLine]->d_name);
...
void nfo(sfmPart *part, int mCol){
    mvwchgat(part->sWind.win, part->sWind.currentLine - part->sWind.top + 1, 1, mCol / 2 - 2, A_REVERSE, 0, NULL);
    ...
    mvprintw(0, mCol - 15, "|%d/%d", part->sWind.currentLine + 1, part->sFold.numbOfFiles);
}
    ...
    while (tmp){
        if (tmp[0] == 'U'){
            ...
            if ((tmp[0]=='c')&&(tmp[1]=='f'))
                ...
                strcat(outCmd, fPart->sFold.files[fPart->sWind.currentLine]->d_name);
...
void saveConf(sfmPart *fPart, sfmPart *sPart){
    ...
    f = fopen(fTmp, "w");
    fprintf(f, "[Main]\nlPath = %s ;\nrPath = %s ;\nlHid = %d ;\nrHid = %d ;\n",
               fPart->sFold.path, sPart->sFold.path, fPart->sFold.showHidden, sPart->sFold.showHidden);
    fclose(f);
}
...
void showCBinds(sfmPart *part, int mRow, sfmBind **ubinds, int numbOfBinds){
    ...
    while (1){
        ...
        while ((tY < mRow - 4) && (i + tY < numbOfBinds)){
            mvwprintw(part->sWind.win, tY + 1, 1, "%c = %s", ubinds[i + tY]->key, ubinds[i + tY]->cmd);
            ++tY;
        }
        move(mRow - 1, 0);
        wrefresh(part->sWind.win);
